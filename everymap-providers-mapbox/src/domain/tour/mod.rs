    ...
    async fn optimize_tour(&self, stops: &[Coordinate], _options: &TourOptions) -> EveryMapResult<TourResponse> {
        if stops.len() < 2 {
            return Err(everymap_core::error::EveryMapError::provider(
                "mapbox", "INVALID_INPUT", "At least 2 stops required for tour optimization"
            ));
        }

        let coords: String = stops.iter()
            .map(|c| format!("{},{}", c.lng, c.lat))
            .collect::<Vec<_>>()
            .join(";");
        let url = format!("{}/optimized-trips/v1/mapbox/driving/{}", self.base_url, coords);
        ...
        let mut waypoints: Vec<(usize, Option<Coordinate>)> = result.waypoints.into_iter()
            .map(|wp| {
                let coord = wp.location.as_ref().and_then(|loc| {
                    if loc.len() >= 2 {
                ...
                let idx = wp.waypoint_index.unwrap_or(0) as usize;
                (idx, coord)
            })
            .collect();
        waypoints.sort_by_key(|(idx, _)| *idx);

        let tour_stops: Vec<TourStop> = waypoints.into_iter()
            .map(|(_, coord)| TourStop {
                coordinate: coord.unwrap_or(Coordinate::ORIGIN),
                arrival_time: None,
                departure_time: None,
                duration: None,
        ...
        let (total_distance, total_duration) = result.trips.first()
