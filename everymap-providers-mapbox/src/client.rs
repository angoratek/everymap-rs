...
/// constructs its own full URL per the MapBox API specification.
...
impl MapBoxClient {
    ...
    pub async fn request(&self, builder: reqwest::RequestBuilder) -> EveryMapResult<reqwest::Response> {
        ...
        if status.is_success() {
            ...
            if status_code == 429 {
                Err(EveryMapError::rate_limited("mapbox", retry_after))
            } else {
...
fn redact_api_key(url: &str) -> String {
    ...
    for param in &["access_token", "key", "apiKey"] {
...
mod tests {
    ...
    fn test_redact_access_token() {
        let url = "https://api.mapbox.com/geocoding/v6/forward?q=Berlin&access_token=pk.secret123";
        assert_eq!(redact_api_key(url), "https://api.mapbox.com/geocoding/v6/forward?q=Berlin&access_token=***");
    }

    #[test]
    fn test_redact_access_token_with_ampersand() {
        let url = "https://api.mapbox.com/geocoding/v6/forward?q=Berlin&access_token=pk.secret123&limit=5";
        assert_eq!(redact_api_key(url), "https://api.mapbox.com/geocoding/v6/forward?q=Berlin&access_token=***&limit=5");
    }

    #[test]
    fn test_redact_no_key() {
        let url = "https://api.mapbox.com/geocoding/v6/forward?q=Berlin";
