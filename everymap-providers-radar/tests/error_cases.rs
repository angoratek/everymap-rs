...
async fn test_unsupported_isoline() {
    let result = RadarIsoline.get_isoline(
...
async fn test_unsupported_traffic() {
    let result = RadarTraffic.get_traffic(
        &everymap_core::types::Coordinate::ORIGIN,
...
async fn test_unsupported_tiling() {
    let result = RadarTileProvider.get_tile(
        10, 5, 3,
...
async fn test_unsupported_positioning() {
    let result = RadarPositioner.get_position(
        &everymap_core::domains::positioning::PositioningOptions::default(),
    ).await;
    assert!(result.is_err());
}
...
async fn test_unsupported_attributes() {
    let result = RadarAttributeProvider.get_attributes(
        &everymap_core::domains::attributes::AttributeOptions::default(),
    ).await;
    assert!(result.is_err());
}
...
async fn test_unsupported_imaging() {
    let result = RadarMapImageProvider.get_image(
        &everymap_core::types::Coordinate::ORIGIN,
        10, (600, 400),
