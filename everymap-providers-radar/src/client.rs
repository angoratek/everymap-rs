...
///
/// Handles authentication via the `Authorization` header. Each domain module
...
impl RadarClient {
    ...
    pub async fn request(&self, builder: reqwest::RequestBuilder) -> EveryMapResult<reqwest::Response> {
        ...
        if status.is_success() {
            ...
            if status_code == 429 {
                Err(EveryMapError::rate_limited("radar", retry_after))
            } else {
...
/// Redact auth-related values from a URL string for verbose output.
fn redact_auth_header(url: &str) -> String {
    let mut result = url.to_string();
    for param in &["apiKey", "key", "access_token", "Authorization"] {
