...
mod tests {
    ...
    fn get_google_api_key() -> Option<String> {
        std::env::var("EVERYMAP_GOOGLE_API_KEY").ok()
            .or_else(|| std::env::var("EVERYMAP_API_KEY").ok())
    }

    fn get_tomtom_api_key() -> Option<String> {
        std::env::var("EVERYMAP_TOMTOM_API_KEY").ok()
            .or_else(|| std::env::var("EVERYMAP_API_KEY").ok())
    }

    fn get_mapbox_api_key() -> Option<String> {
        std::env::var("EVERYMAP_MAPBOX_API_KEY").ok()
            .or_else(|| std::env::var("EVERYMAP_API_KEY").ok())
    }
    ...
    async fn test_here_geocode_live() {
        ...
        let res = geocoder.geocode("Brandenburg Gate, Berlin", &opts).await
            .expect("HERE geocode request failed");

        assert!(!res.items.is_empty(), "Expected at least one result");
        let first = &res.items[0];
        assert!(first.coordinate.lat > 52.0 && first.coordinate.lat < 53.0, "Expected Berlin latitude");
    }
    ...
    async fn test_here_route_live() {
        ...
        assert!(res.routes[0].distance > 100_000.0, "Berlin-Paris should be >100km");
    }
