...
use everymap_providers_here::domain::tour::{
    HereTourPlanner, TourProblem, Fleet, Profile, VehicleType, VehicleCosts,
...
fn make_client(api_key: &str) -> Arc<HereClient> {
    let auth = Arc::new(ApiKeyProvider::new(api_key.to_string(), "apiKey".to_string()));
...
async fn live_geocode_berlin() {
    ...
    match res {
        Ok(res) => {
            ...
            assert!(!res.items.is_empty(), "Geocode should return at least one result");
            let first = &res.items[0];
            assert!(first.coordinate.lat > 52.0 && first.coordinate.lat < 53.0,
                "Berlin latitude should be around 52.5, got {}", first.coordinate.lat);
            assert!(first.coordinate.lng > 13.0 && first.coordinate.lng < 14.0,
                "Berlin longitude should be around 13.4, got {}", first.coordinate.lng);
            println!("[geocode] first result: {:?} at {}", first.title, first.coordinate);
        }
        Err(e) => {
...
async fn live_route_berlin_to_munich() {
    ...
    match res {
        Ok(res) => {
            ...
            assert!(!res.routes.is_empty(), "Route calculation should return at least one route");
            let route = &res.routes[0];
            assert!(route.distance > 0.0, "Route distance should be > 0, got {}", route.distance);
            assert!(route.duration > 0.0, "Route duration should be > 0, got {}", route.duration);
            // Berlin to Munich is roughly 580-600 km by car
            assert!(route.distance > 400_000.0 && route.distance < 800_000.0,
                "Berlin-Munich distance should be roughly 500-700 km, got {} m", route.distance);
            println!("[route] distance: {:.1} km, duration: {:.0} s ({:.1} h)",
                route.distance / 1000.0, route.duration, route.duration / 3600.0);
        }
        Err(e) => {
...
async fn live_isoline_berlin_5km() {
    ...
    match res {
        Ok(res) => {
            ...
            assert!(!res.isolines.is_empty(), "Isoline should return at least one result");
            let iso = &res.isolines[0];
            assert!(!iso.polygon.is_empty(), "Isoline polygon should not be empty");
            println!("[isoline] polygon points: {}, range: {:?}", iso.polygon.len(), iso.range);
        }
        Err(e) => {
...
async fn live_matching_gps_trace() {
    ...
    match res {
        Ok(res) => {
            println!("[matching] matched points: {}, distance: {:.1} m",
                res.matched_points.len(), res.distance);
            assert!(!res.matched_points.is_empty(), "Matching should return matched points");
...
async fn live_traffic_berlin() {
    ...
    match res {
        Ok(res) => {
            println!("[traffic] flows: {}, incidents: {}", res.flows.len(), res.incidents.len());
            ...
            if !res.flows.is_empty() {
                ...
                println!("[traffic] first flow - speed: {:?}, jam_factor: {:?}",
...
async fn live_positioning_empty_observations() {
    ...
    match res {
        Ok(res) => {
            println!("[positioning] coordinate: {}, accuracy: {:?}",
        ...
        Err(e) => {
            ...
            eprintln!("[positioning] ERROR (expected with empty observations): {:?}", e);
...
async fn live_tour_3_stops_berlin() {
    ...
    let problem = TourProblem {
            ...
            types: vec![VehicleType {
                ...
                shifts: vec![VehicleShift {
                    start: ShiftStart {
                        ...
                        earliest: None,
                        location: Some(TourLocation { lat: 52.5200, lng: 13.4050 }),
                    },
                    ..Default::default()
                }],
        ...
        plan: Plan {
            jobs: stops.iter().enumerate().map(|(i, coord)| Job {
                id: format!("stop_{}", i),
                tasks: JobTasks {
    ...
    match res {
        Ok(res) => {
            println!("[tour] stops: {}, total_distance: {:?}, total_duration: {:?}",
                res.stops.len(), res.total_distance, res.total_duration);
            assert!(!res.stops.is_empty(), "Tour should return at least one stop");
            if let Some(dist) = res.total_distance {
                assert!(dist > 0.0, "Tour total distance should be > 0, got {}", dist);
            }
        }
        Err(e) => {
...
async fn live_tiling_zoom14_berlin() {
    ...
    match res {
        Ok(res) => {
            println!("[tiling] data bytes: {}, content_type: {:?}",
                res.data.len(), res.content_type);
            assert!(!res.data.is_empty(), "Tile data should not be empty");
            assert!(res.content_type.is_some(), "Tile should have a content-type");
        }
        Err(e) => {
...
async fn live_attributes_roads_berlin() {
    ...
    match res {
        Ok(res) => {
            println!("[attributes] response keys: {:?}",
            ...
            assert!(res.data.is_object(), "Attributes response should be a JSON object");
            if let Some(obj) = res.data.as_object() {
                assert!(
                    obj.contains_key("geometries") || obj.contains_key("features") || obj.contains_key("type"),
...
async fn live_imaging_static_map_berlin() {
    ...
    match res {
        Ok(res) => {
            println!("[imaging] data bytes: {}, content_type: {:?}",
                res.data.len(), res.content_type);
            assert!(!res.data.is_empty(), "Image data should not be empty");
            assert!(res.content_type.is_some(), "Image should have a content-type");
            let ct = res.content_type.as_deref().unwrap_or("");
            assert!(ct.starts_with("image/"), "Content type should be an image type, got: {}", ct);
        }
        Err(e) => {
