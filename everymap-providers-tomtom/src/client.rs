...
/// constructs its own full URL per the TomTom API specification.
...
impl TomTomClient {
    ...
    pub async fn request(&self, builder: reqwest::RequestBuilder) -> EveryMapResult<reqwest::Response> {
        ...
        if status.is_success() {
            ...
            if status_code == 429 {
                Err(EveryMapError::rate_limited("tomtom", retry_after))
            } else {
...
fn redact_api_key(url: &str) -> String {
    ...
    for param in &["key", "apiKey", "access_token"] {
...
mod tests {
    ...
    #[test]
    fn test_redact_api_key_tomtom() {
        let url = "https://api.tomtom.com/search/2/geocode/Berlin.json?key=secret123";
        assert_eq!(redact_api_key(url), "https://api.tomtom.com/search/2/geocode/Berlin.json?key=***");
    }

    #[test]
    fn test_redact_api_key_with_ampersand() {
        let url = "https://api.tomtom.com/search/2/geocode/Berlin.json?key=secret123&limit=5";
        assert_eq!(redact_api_key(url), "https://api.tomtom.com/search/2/geocode/Berlin.json?key=***&limit=5");
    }

    #[test]
    fn test_redact_no_key() {
        let url = "https://api.tomtom.com/search/2/geocode/Berlin.json";
