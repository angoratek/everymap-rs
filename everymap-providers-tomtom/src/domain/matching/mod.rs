...
impl RouteMatcher for TomTomRouteMatcher {
    async fn match_route(
        ...
        if options.transport_mode.is_some() {
            log::warn!("TomTom Snap to Roads API does not support transport_mode; ignoring");
        }
        if options.heading.is_some() {
            log::warn!("TomTom Snap to Roads API does not support heading; ignoring");
        }
        if options.departure_time.is_some() {
            log::warn!("TomTom Snap to Roads API does not support departure_time; ignoring");
        }
        if !options.avoid.is_empty() {
            log::warn!("TomTom Snap to Roads API does not support avoid restrictions; ignoring");
        }
