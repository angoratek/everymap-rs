...
impl TourPlanner for TomTomTourPlanner {
    async fn optimize_tour(
        ...
        let waypoints: Vec<serde_json::Value> = stops
            .iter()
            .map(|c| {
                serde_json::json!({
        ...
        let tour_stops: Vec<TourStop> = result
            .optimized_order
            .into_iter()
            .filter_map(|index| {
                let stop_index = index as usize;
                stops.get(stop_index).map(|c| TourStop {
                    coordinate: *c,
                    arrival_time: None,
                    departure_time: None,
                    duration: None,
        ...
        let (total_distance, total_duration) = result
            ...
            .and_then(|s| {
                s.route_summary.map(|rs| {
                    (
                        rs.length_in_meters.unwrap_or(0.0),
                        rs.travel_time_in_seconds.unwrap_or(0.0),
                    )
                })
            })
            .unwrap_or((0.0, 0.0));
